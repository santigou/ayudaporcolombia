...
interface ModeratorRequestItem {
  id: string;
  createdAt: string;
  user: { id: string; name: string; email: string; contactInfo?: string | null };
}
  ...
  async function loadAll() {
    ...
    try {
      const [points, requests] = await Promise.all([
        api.get<Point[]>("/moderator/points/pending"),
      ...
      <div className="mt-3 flex gap-2 border-b border-gray-200">
        <button
          ...
          className={`px-3 py-2 text-sm font-medium border-b-2 ${
            tab === "puntos" ? "border-brand text-brand-dark" : "border-transparent text-gray-500"
          }`}
        >
        ...
        <button
          ...
          className={`px-3 py-2 text-sm font-medium border-b-2 ${
            tab === "solicitudes" ? "border-brand text-brand-dark" : "border-transparent text-gray-500"
          }`}
        >
          Solicitudes de moderador ({pendingRequests.length})
        </button>
      </div>
      ...
      {loading ? (
        ...
        <ul className="mt-4 flex flex-col gap-3">
          {pendingPoints.length === 0 && (
            <p className="text-sm text-gray-500">No hay puntos pendientes.</p>
          )}
          {pendingPoints.map((point) => (
            <li key={point.id} className="rounded-md border border-gray-200 p-3">
              ...
              <p className="mt-2 text-xs text-gray-500">
                Código de verificación: <span className="font-mono font-bold">{point.verificationCode}</span>
              </p>
              <p className="text-xs text-gray-500">
                Creado por: {point.createdBy?.name} ({point.createdBy?.email})
                {point.createdBy?.contactInfo ? ` — contacto: ${point.createdBy.contactInfo}` : ""}
              </p>
              <p className="text-xs text-gray-500">Contacto del punto: {point.contactInfo}</p>
              <div className="mt-2 flex gap-2">
                <button
                  onClick={() => reviewPoint(point.id, "approve")}
                  className="rounded-md bg-emerald-600 text-white px-3 py-1.5 text-sm font-medium"
                >
                  Aprobar
                </button>
                <button
                  onClick={() => reviewPoint(point.id, "reject")}
          ...
          {pendingRequests.map((req) => (
            <li key={req.id} className="rounded-md border border-gray-200 p-3">
              ...
              {req.user.contactInfo && (
                <p className="text-xs text-gray-500">Contacto: {req.user.contactInfo}</p>
              )}
              <div className="mt-2 flex gap-2">
                <button
