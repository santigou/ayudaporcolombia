...
function StepFooter({ children }: { children: ReactNode }) {
  return <div className="shrink-0 border-t border-gray-200 bg-white px-4 py-3">{children}</div>;
}
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-8 text-center">
        <div className="mx-auto w-full max-w-md">
          ...
          <h1 className="text-2xl font-bold text-gray-900">Bienvenido a Ayuda por Colombia</h1>
      ...
      <div className="shrink-0 border-t border-red-100 bg-white px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-3">
        <button
          type="button"
    ...
    <div className="flex h-full flex-col">
      <div className="flex-1 overflow-y-auto px-6 py-8">
        <div className="mx-auto max-w-md text-center">
          <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-blue-100 text-2xl">📍</div>
          <h2 className="text-xl font-bold text-gray-900">Tu ubicación</h2>
          <p className="mt-2 text-gray-600">
          ...
          <div className="mt-5 grid gap-3 text-left">
            <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3">
              <p className="text-sm font-semibold text-emerald-800">✓ Si aceptas</p>
              <p className="text-sm text-emerald-700">El mapa se centra en tu zona y verás los puntos cercanos a ti.</p>
            </div>
            <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
              <p className="text-sm font-semibold text-gray-700">✗ Si rechazas</p>
  ...
  return (
    <div className="relative h-full w-full">
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 flex justify-center p-3">
        <div className="pointer-events-auto max-w-md rounded-lg bg-white/95 px-4 py-2 text-center text-sm shadow-md">
          <p className="font-semibold text-gray-900">Explora el mapa</p>
      ...
      <div className="absolute inset-x-0 bottom-0 flex items-center justify-between gap-2 p-3">
        <span className="rounded-full bg-white/90 px-3 py-1.5 text-[11px] font-medium text-gray-600 shadow">
          Puntos de ejemplo (no reales)
        </span>
        <button onClick={onSkip} className="rounded-full bg-white/90 px-4 py-1.5 text-sm font-medium text-gray-700 shadow hover:bg-white">
          Omitir
        </button>
      </div>
    </div>
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center px-6 text-center">
      <div className="mx-auto max-w-md">
        ...
        <div className="mt-7 flex flex-col items-center gap-3">
          <Link to="/registro" onClick={onComplete} className="rounded-full bg-brand px-6 py-3 font-semibold text-white shadow-lg hover:bg-brand-dark">
            Crear cuenta
          </Link>
          <Link to="/login" onClick={onComplete} className="text-sm font-medium text-brand-dark hover:underline">
